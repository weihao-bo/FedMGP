...
class MultiheadAttentionWithVis(nn.MultiheadAttention):
    """MultiheadAttention with attention visualization support."""
    ...
    def save_attention_probs(self, attn_probs):
        self.attn_probs = attn_probs

    def save_attention_grad(self, attn_grad):
    ...
    def forward(self, query, key, value, key_padding_mask=None, need_weights=True, attn_mask=None):
        ...
        if need_weights and attn_output_weights is not None and query.requires_grad:
            self.save_attention_probs(attn_output_weights)
            if attn_output_weights.requires_grad:
                attn_output_weights.register_hook(self.save_attention_grad)
...
class ModifiedResNet_FEDOPT(nn.Module):
    """
    A ResNet class that is similar to torchvision's but contains the following changes:
    - There are now 3 "stem" convolutions as opposed to 1, with an average pool instead of a max pool.
    - Performs anti-aliasing strided convolutions, where an avgpool is prepended to convolutions with stride > 1
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None):
        ...
        self.attn = MultiheadAttentionWithVis(d_model, n_head)
    ...
    def save_attention_probs(self, attn_probs):
        self.attn_probs = attn_probs
        if hasattr(self.attn, 'save_attention_probs'):
            self.attn.save_attention_probs(attn_probs)

    def save_attention_grad(self, attn_grad):
        self.attn_grad = attn_grad
        if hasattr(self.attn, 'save_attention_grad'):
            self.attn.save_attention_grad(attn_grad)

    def attention(self, x: torch.Tensor):
        self.attn_mask = self.attn_mask.to(dtype=x.dtype, device=x.device) if self.attn_mask is not None else None
        attn_output, attn_weights = self.attn(x, x, x, need_weights=True, attn_mask=self.attn_mask)

        if x.requires_grad and attn_weights is not None:
            self.save_attention_probs(attn_weights)
            if attn_weights.requires_grad:
                attn_weights.register_hook(self.save_attention_grad)
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        elif current_trainer == 'MaPLe' or current_trainer == 'fedmaple':
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int,
        ...
        self.is_fedmgp = design_details.get('trainer', '') == 'FedMGP'
        ...
        if self.VPT_shallow:
            self.num_prompts_vision = 1
            if self.is_fedmgp and 'vision_num_prompts' in design_details:
                self.num_prompts_vision = design_details["vision_num_prompts"]

            n_ctx = design_details["vision_ctx"]

            if self.num_prompts_vision > 1:
                # Initialize multiple vision prompts with independent random seeds
                ctx_vectors = torch.empty(self.num_prompts_vision, n_ctx, width)
                base_seed = int(time.time()) % 10000

                for i in range(self.num_prompts_vision):
                    prompt_seed = base_seed + i * 1000 + 42
                    torch.manual_seed(prompt_seed)
                    nn.init.normal_(ctx_vectors[i], std=0.02)

                self.VPT = nn.Parameter(ctx_vectors, requires_grad=True)
            else:
    ...
    def forward(self, x: torch.Tensor, prompt_idx=0):
        ...
        # Attach visual prompts after positional embeddings (only trainable params in image encoder)
        if self.VPT_shallow:
            if hasattr(self, 'num_prompts_vision') and self.num_prompts_vision > 1:
                if prompt_idx >= self.num_prompts_vision:
                    prompt_idx = 0
                selected_prompt = self.VPT[prompt_idx].to(x.dtype)
                visual_ctx = selected_prompt.expand(x.shape[0], -1, -1)
            else:
                visual_ctx = self.VPT.to(x.dtype).expand(x.shape[0], -1, -1)
...
class ModifiedVisionTransformer(nn.Module):
    """VisionTransformer variant that preserves all token features.
    Output shape: [sequence_length, batch_size, output_dim].
    """
    ...
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "CoOp2":
                ...
                self.visual = ModifiedResNet_FEDOPT(
                    layers=vision_layers,
                    output_dim=embed_dim,
                    heads=vision_heads,
                    input_resolution=image_resolution,
            ...
            elif trainer == "MaPLe" or trainer == "fedmaple":
            ...
            elif trainer == "FEDOPT" or trainer == "FedOPT":
                self.visual = ModifiedVisionTransformer(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
        ...
        if trainer == "FedTPG":
            ...
            # hyper-parameter if need to add prompt embeddings inside to the input
            # of transformer block or not:
            prompt_till_layer_text = design_details.get('language_depth', 0)
